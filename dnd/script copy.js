...
// Define recalc function at the top
const recalc = () => {
...
(() => {
  let spellsData = [];
  let weaponsData = [];
  // Remove currentRacialBonus declaration from here since it's now global
  ...
  const updateRace = () => {
    ...
    // Update the global currentRacialBonus
...
// Roll all abilities button click handler
...
document.getElementById("rollDiceButton").addEventListener("click", rollAllAbilities);
  ...
  const rollAbilities = () => {
    ["str", "dex", "con", "int", "wis", "cha"].forEach(ability => {
      document.getElementById(ability).value = roll4d6DropLowest();
    });
    recalc();
  };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  scoreMethodSelect.addEventListener('change', () => {
    const method = scoreMethodSelect.value;
    const rollDiceButton = document.getElementById("rollDiceButton");
    const remainingPointsDisplay = document.getElementById("remainingPointsDisplay");
    ...
    if (method === "Standard Array") {
      // Set standard array values
      document.getElementById("str").value = 15;
      document.getElementById("dex").value = 14;
      document.getElementById("con").value = 13;
      document.getElementById("int").value = 12;
      document.getElementById("wis").value = 10;
      document.getElementById("cha").value = 8;
    } else if (method === "Point Buy") {
      remainingPointsDisplay.style.display = "block";
      updatePointBuyRemaining();
    } else if (method === "Roll Dice") {
      console.log("Setting roll dice button display to block"); // Debug log
      rollDiceButton.style.display = "inline-block";
    }
    
    // If you have a saveState function, call it here
  ...
  if (rollDiceButton) {
    rollDiceButton.addEventListener("click", () => {
      ["str", "dex", "con", "int", "wis", "cha"].forEach(stat => {
        const rolls = Array.from({ length: 4 }, () => Math.floor(Math.random() * 6) + 1);
        rolls.sort((a, b) => b - a);
        const value = rolls.slice(0, 3).reduce((sum, roll) => sum + roll, 0);
    ...
    fetch("weapons.json")
      ...
      .then(data => {
        ...
        datalist.innerHTML = weaponsData
          .map(
            w =>
              `<option value="${w.name}" data-damage="${w.damage}" data-properties="${w.properties}"></option>`
          )
          .join("");
      })
  ...
  const addWeaponRow = () => {
    ...
    nameInput.addEventListener("input", function () {
      const weapon = weaponsData.find(
        w => w.name.toLowerCase() === this.value.toLowerCase()
      );
      if (weapon) {
