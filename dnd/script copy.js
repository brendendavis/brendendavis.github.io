...
document.getElementById('equipmentTable').addEventListener('input', function(event) {
  if (event.target.classList.contains('equipment-name')) {
    ...
    // Show item context if it exists
...
function showEquipmentContext(details, targetElement) {
  ...
  // If clicking the same item again, toggle it off
...
/**
 * hideEquipmentContext()
 * Fades out and hides the context window.
 */
...
async function loadArmorOptions() {
  try {
          ...
          if (armorData[category]) {
              ...
              armorTable["Armor"].forEach((armor, index) => {
                  if (!addedArmor.has(armor)) {
                      let acValue = armorTable["Armor Class (AC)"][index].match(/\d+/);
                      const option = document.createElement("option");
                      option.value = armor;
                      option.dataset.ac = acValue ? parseInt(acValue[0]) : 10;
                      option.textContent = `${armor} (AC: ${armorTable["Armor Class (AC)"][index]})`;
      ...
      if (armorData["Shield"] && !addedArmor.has("Shield")) {
          ...
          shieldOption.value = shieldData["Armor"][0];
          shieldOption.dataset.ac = "+2";
          shieldOption.textContent = `${shieldData["Armor"][0]} (AC: +2)`;
