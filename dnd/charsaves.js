 ...
 * Collect all sheet data so it can be reused for saving or
 * switching between player tabs without losing context.
 **********************************************************/
function snapshotCharacterState(options = {}) {
    ...
    if (includeTimestamp) {
        characterData.lastSaved = new Date().toISOString();
    }
    ...
    equipmentRows.forEach(row => {
        ...
        equipmentData.push({
            ...
            cost: cells[1].querySelector('input')?.value || "",
            quantity: cells[2].querySelector('input')?.value || "",
            weight: cells[3].querySelector('input')?.value || "",
            equipped: cells[4].querySelector('input[type="checkbox"]')?.checked || false
        });
    });
    ...
    weaponRows.forEach(row => {
        const cells = row.querySelectorAll('td');
        gatheredWeapons.push({
            weaponName: cells[0].querySelector('input')?.value || "",
            attackBonus: cells[1].querySelector('input')?.value || "",
            damage: cells[2].querySelector('input')?.value || "",
            properties: cells[3].querySelector('input')?.value || ""
        });
    });
...
/**********************************************************
 * saveCharacter()
 * Gathers all fields and saves to localStorage.
 **********************************************************/
function saveCharacter() {
    const characterData = snapshotCharacterState({ includeTimestamp: true });
    try {
...
/**********************************************************
 * applyCharacterState()
 * Rebuilds the sheet from a snapshot.
 **********************************************************/
function applyCharacterState(character) {
    if (!character) return;
    const applyValueWithEvents = (element, value) => {
        if (!element) return;
        const isFormElement =
    ...
    Object.keys(character).forEach(key => {
        applyValueWithEvents(document.getElementById(key), character[key]);
    });
...
window.snapshotCharacterState = snapshotCharacterState;
window.applyCharacterState = applyCharacterState;
