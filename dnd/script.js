...
(() => {
  ...
  const updateProgress = (spellLevel, max) => {
...
// Global variable to store equipment names
let equipmentData = [];
let equipmentContextMap = {};
let equipmentDataStore = {};

// Load equips.json when the page loads
document.addEventListener('DOMContentLoaded', function() {
  fetch('equips.json')
    ...
    .then(data => {
      ...
      // Process the equipment context details from the "content" array
...
// Function to populate the datalist with equipment items
...
// Initialize Equipment Data from the JSON
function initializeEquipmentData(adventuringGear) {
  ...
  console.log("Initialized Equipment Data Store:", equipmentDataStore);
}

// Process Equipment Context from JSON
...
// Event Delegation for Showing Context Window
...
// Function to remove an equipment row
function removeEquipmentRow(button) {
  const row = button.closest("tr");
  if (row) row.remove();
}

// Event Delegation for Equipment Input Changes
document.getElementById('equipmentTable').addEventListener('input', function(event) {
  if (event.target.classList.contains('equipment-name')) {
    const itemName = normalizeEquipmentName(event.target.value);
...
// Show and Hide Equipment Context
// Show Equipment Context with Smooth Transition
function showEquipmentContext(details, targetElement) {
  ...
  // Check if the same item is clicked to toggle visibility
  ...
  // Calculate position relative to the clicked input
  ...
  // Make the context window visible
  contextDiv.style.display = "block";
  contextDiv.style.opacity = 1;

  // Update lastClickedItem to the current element
  lastClickedItem = targetElement;
}


function hideEquipmentContext() {
  const contextDiv = document.getElementById("equipmentContext");
  contextDiv.style.opacity = 0;
  setTimeout(() => {
    contextDiv.style.display = "none";
    lastClickedItem = null; // Reset to allow recalling the context window
